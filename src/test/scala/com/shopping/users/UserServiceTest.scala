...
class UserServiceTest extends BaseTest with PlayJsonSupport {
  ...
  test("should sort users in descending order based on their age") {
      ...
      val mayBeUsers =
        Await.result(service.getUsers(UserParams(None, Some("AGE"), Some("DESC"))), Duration(3, TimeUnit.SECONDS))
  ...
  test("should sort users in ascending order based on their age") {
      ...
      val mayBeUsers =
        Await.result(service.getUsers(UserParams(None, Some("AGE"), Some("ASC"))), Duration(3, TimeUnit.SECONDS))
  ...
  test("should by default sort users in ascending order based on their age") {
      ...
      val mayBeUsers =
        Await.result(service.getUsers(UserParams(None, Some("AGE"), None)), Duration(3, TimeUnit.SECONDS))
